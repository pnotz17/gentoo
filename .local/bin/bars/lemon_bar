...
totalws (){
 ...
 updates() {
	...
	echo "%{F$blueColor}PACMAN: $updates %{F-}"
}
...
alsa() {
    volume=$(amixer sget Master | sed '5!d;s_]..*__g;s_.*\[__')
...
while true; do
	 echo -e "%{l} $(currentws) $(totalws)%{c}%{r}  $(updates) |  $(mail) | $(cpufrequency) |  $(ram) |  $(upspeed) |  $(downspeed) |  $(alsa) |  $(clock) |"
	sleep 1
done &
